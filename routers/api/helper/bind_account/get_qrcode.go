...
func (*BindAccount) GetQRCode(c *gin.Context) {
	...
	if usr.HelperMpayUser != nil {
		...
		if usr.HelperMpayUser.GetType() != models.MpayUserTypeWindows {
			user.DeleteHelper(usr)
		}
	}
	// Create helper user if not exist
	if usr.HelperMpayUser == nil {
		usr.HelperMpayUser = &models.WindowsMpayUser{}
	}
	defer models.DBSave(usr)
	// Try to request code
	uuid, image, protocolErr := usr.HelperMpayUser.QRCodeLoginGetUUID()
	if protocolErr != nil {
